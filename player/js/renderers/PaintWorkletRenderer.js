function determinePaintWorkletScriptPath(currentScript) {
  ...
  if (currentScript.endsWith('/PaintWorkletRenderer.js'))
    return resolveRelative(
      currentScript,
      '../../../build/player/lottie_paintworklet.js'
    );
  const m = currentScript.match(/(^.*lottie)(_[^.])?(\.min)?\.js$/);
  if (!m) return '';
  return m[1] + '_paintworklet.js';
}
const LOTTIE_SCRIPT_SRC = determinePaintWorkletScriptPath(
...
PaintWorkletRenderer.prototype.configAnimation = function(animData, cb) {
  ...
  if (this.renderConfig.className) {
    this.animationItem.wrapper.setAttribute(
      'class',
...
PaintWorkletRenderer.prototype.renderFrame = function(num, forceRender) {
  if (registerProperty)
    this.animationItem.wrapper.style.setProperty(
      '--progress',
      num / this.animationItem.totalFrames
    );
  else
    this.animationItem.wrapper.style.setProperty(
      'object-position',
